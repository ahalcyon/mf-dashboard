...
グループ視点では、グループ内アカウントからグループ外への送金記録を「収入」として扱う。
...
グループ外からの入金が振替としてのみ記録されている場合に支出としてカウントする。
通常トランザクション（expense）として記録されている場合は通常トランザクションを採用する。

### 内部振替（カウントしない）

グループ内→グループ内の振替は収支に含めない。

## 重複除外

同一の振替が複数回記録されることがある。以下のキーで重複除外する：

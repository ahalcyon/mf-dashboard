...
インサイトページでは、取得した家計データをもとに、貯蓄の健全性、収支バランス、支出パターン、投資パフォーマンス、負債状況を分析する。財務健全性スコアや主要な指標に加えて、AIが生成した要約と各項目へのコメントをまとめて確認できる。生成はcrawler側で行い、Webアプリは保存済みの結果を表示する。

<img src="./.github/assets/ai-insight.png" alt="財務インサイト画面" width="600" />

### 資産データのエクスポート

日次の資産総額の推移、資産・負債の内訳、口座残高、保有銘柄の評価損益を、JSONとMarkdownで書き出す。ビルド時に生成する静的ファイルなので、サイドバーからダウンロードしてそのままLLMへ読み込ませられる。
...
flowchart TD
    ...
    O[環境変数 / AWS SSM<br/>認証情報とTOTPシード] --> C
